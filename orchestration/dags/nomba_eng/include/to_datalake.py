...
logging.basicConfig(
    filename=DATALAKE_LOG_FILE,
    format="%(asctime)s:%(levelname)s:%(message)s",
    level=logging.INFO,
)

postgres_src_engine = create_engine(
    f"postgresql+psycopg2://{USER}:{PASS}@{HOST}:{PG_PORT}/{DBNAME}"
)
...
def mongodb_tos3() -> None:
    ...
    users_data = pd.DataFrame(
        collection.find(
            {"updated_at": {"$gt": datetime.now() - timedelta(hours=24)}}
            )
    )
...
def postgresSource_tos3() -> None:
    ...
    for table, table_data in tables.items():
        ...
        logging.info(
            f"Written {len(table_data)}"
            f"records from" f"{table} to staging layer!"
        )
